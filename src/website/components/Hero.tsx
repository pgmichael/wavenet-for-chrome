          ...
          <div className='lg:w-1/2 flex flex-col justify-center gap-6 pt-12 lg:py-12 items-center text-center lg:text-left lg:items-start'>
            <div>
              <span className='flex gap-2 justify-center lg:justify-start'>
                <span className="font-semibold text-xs uppercase bg-green-500 border border-green-600 text-white px-1.5 py-0.5 rounded w-fit">11'000+ users</span>
                <span className="font-semibold text-xs uppercase bg-yellow-500 border border-yellow-600 text-white px-1.5 py-0.5 rounded w-fit">4.8/5 stars</span>
              </span>
              <h1 className='text-4xl font-bold max-w-screen-md mt-3'>
                Transform highlighted text into high-quality natural sounding audio
              </h1>
              <p className='text-neutral-500 text-lg mt-2 max-w-2xl'>
                Wavenet for Chrome is an extension that allows you to use Google Cloud's text to speech API to listen or download audio from any website
              </p>
            </div>
            <div className='flex gap-2'>
              ...
              <Button ping={!playing && !played} Icon={playing ? StopCircle : Play} type='primary' className='w-fit' onClick={play}>
                {playing ? 'Stop playing' : 'Listen to sample'}
              </Button>
            </div>
          </div>
          <div className='flex justify-center items-center lg:w-1/2 px-4 md:px-8 lg:px-0'>
            <img src={new URL("../assets/images/screenshot.png", import.meta.url).toString()} className='cursor-not-allowed w-2/3 lg:w-auto min-w-full shadow-xl rounded-lg border-2 border-neutral-200' style={{
            marginBottom: '-50%',
          }} />
          </div>
        </div>
